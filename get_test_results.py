def run_model_on_test(path_results, model="TLSTM", hidden_sz=1024, fc_sz=1024,
    ...
    lstm = biTLSTM(input_sz, hidden_sz, fc_sz, output_sz,
                   dropout_rate, discount, bidirectional, survival, embed_sz)
    ...
    Y_true_concat = np.concatenate(
        [Y_true_test[i].reshape(-1, output_sz) for i in range(len(Y_true_test))], axis=0)
    Y_logit_concat = np.concatenate(
        [Y_pred_test[i].reshape(-1, output_sz) for i in range(len(Y_pred_test))], axis=0)
