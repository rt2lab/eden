...
class biTLSTM(nn.Module):  # the class inherits from nn.Module
    ...
    def __init__(self, input_sz: int, hidden_sz: int, fc_sz: int, output_sz: int,
                 dropout_rate: float, discount="log", bidirectional=False,
                 survival=False, embed_sz=28):
        super().__init__()
        ...
        if(bidirectional):
            self.W_backward = nn.Parameter(
                torch.Tensor(embed_sz, hidden_sz * 4))
            self.U_backward = nn.Parameter(
                torch.Tensor(hidden_sz, hidden_sz * 5))
    ...
    def init_weights(self):
        ...
        nn.init.xavier_normal_(
            self.W_forward, gain=nn.init.calculate_gain("relu"))
        nn.init.xavier_normal_(
            self.U_forward, gain=nn.init.calculate_gain("relu"))
        nn.init.normal_(self.bias_forward)
        if(self.bidirectional):
            nn.init.xavier_normal_(
                self.W_backward, gain=nn.init.calculate_gain("relu"))
            nn.init.xavier_normal_(
                self.U_backward, gain=nn.init.calculate_gain("relu"))
    ...
    def forward(self, delta, batch_xs, init_states=None):
        ...
        for t in range(seq_sz):
            ...
            gates = x_t @ self.W_forward + \
                h_t @ self.U_forward[:, :4*HS] + self.bias_forward[:4*HS]
            i_t, f_t, g_t, o_t = (
            ...
            short_term_memory = torch.tanh(
                c_t @ self.U_forward[:, 4*HS:5*HS] + self.bias_forward[4*HS:5*HS])
            discounted_short_term = short_term_memory * \
                delta_t.expand_as(short_term_memory)
            ...
            ht_forward_seq = torch.cat(
                (ht_forward_seq, h_t.unsqueeze(dim=1)), dim=1)

        # Backward LSTM
        if(self.bidirectional):
            ht_backward_seq = torch.zeros(
                batch_xs.shape[0], 0, self.hidden_size)
            for t in range(seq_sz-1, -1, -1):
                ...
                gates = x_t @ self.W_backward + \
                    h2_t @ self.U_backward[:, :4*HS] + \
                    self.bias_backward[:4*HS]
                i_t, f_t, g_t, o_t = (
                    torch.sigmoid(gates[:, :HS]),
                    torch.sigmoid(gates[:, HS:HS*2]),
                    torch.tanh(gates[:, HS*2:HS*3]),
                ...
                short_term_memory = torch.tanh(
                    c2_t @ self.U_backward[:, 4*HS:5*HS] + self.bias_backward[4*HS:5*HS])
                discounted_short_term = short_term_memory * \
                    delta_t.expand_as(short_term_memory)
                ...
                ht_backward_seq = torch.cat(
                    (ht_backward_seq, h2_t.unsqueeze(dim=1)), dim=1)
