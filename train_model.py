def train_model(path_results, model = "TLSTM",hidden_sz = 1024 ,fc_sz = 1024,
                  ...
                  training_epochs = 100, embed_sz = 25,attention_medical_code = True, attention_medical_size = 25,
    ...
    data_train_batches = np.load(path_results+"/data/data_train" +".npy",allow_pickle=True)
    data_val_batches = np.load(path_results+"/data/data_val" +".npy",allow_pickle=True)
    ...
    elapsed_train_batches = np.load(path_results+"/data/elapsed_train" +".npy",allow_pickle=True)
    elapsed_val_batches = np.load(path_results+"/data/elapsed_val" +".npy",allow_pickle=True)
    ...
    labels_train_batches = np.load(path_results+"/data/labels_train" +".npy",allow_pickle=True)
    labels_val_batches = np.load(path_results+"/data/labels_val" +".npy",allow_pickle=True)
    ...
    for epoch in range(training_epochs): 
        ...
        previous_loss, cost_traink,_,Y_truek,Y_predk,_,_,loss_tot_traink,_ = loop_over_all_batches(
              lstm,
              data_train_batches,labels_train_batches,elapsed_train_batches, optimizer, criterion,
              ...
              loss1_weight = loss1_weight,loss2_weight = loss2_weight,loss3_weight = loss3_weight,loss4_weight=loss4_weight)
        ...
        _, _,cost_valk,_,_,Y_true_valk,Y_pred_valk,_,loss_tot_valk = loop_over_all_batches(
              lstm,
    ...
    if(save_results):
        ...
        arr_total_cost_train = np.column_stack((np.arange(1,training_epochs+1),np.hstack(total_cost_train)))
        pd.DataFrame(arr_total_cost_train, columns = ["epoch","value"]).to_csv(path_results + "/data/total_cost_train.csv")
        
        arr_total_cost_val = np.column_stack((np.arange(1,training_epochs+1),np.hstack(total_cost_val))) 
        pd.DataFrame(arr_total_cost_val, columns = ["epoch","value"]).to_csv(path_results + "/data/total_cost_val.csv")
                    
        #Save a more compact form of the total logit prediction for the last epoch    
        Y_true_concat = np.concatenate([Y_true_valk[i].reshape(-1,output_sz) for i in range(len(Y_true_valk))], axis=0)
        Y_logit_concat = np.concatenate([Y_pred_valk[i].reshape(-1,output_sz) for i in range(len(Y_true_valk))], axis=0)
        
        #Total loss 
        total_loss_train = np.concatenate([loss_tot_train[i].reshape(-1,4) for i in range(len(loss_tot_train))], axis=0)
        total_loss_val = np.concatenate([loss_tot_val[i].reshape(-1,4) for i in range(len(loss_tot_val))], axis=0)
        pd.DataFrame(total_loss_train, columns = ["l1","l2","l3","l4"]).to_csv(path_results + "/data/detail_cost_train.csv")
        pd.DataFrame(total_loss_val, columns = ["l1","l2","l3","l4"]).to_csv(path_results + "/data/detail_cost_val.csv")
