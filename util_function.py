...
def loop_over_all_batches(lstm,
    ...
    if(compute_train_loss or compute_train_pred or compute_optimizer):
        ...
        for i in range(number_batches_train):
            print("Batch", i)
            batch_xs, batch_ys, batch_ts = data_train_batches[
                i], labels_train_batches[i], elapsed_train_batches[i]

            batch_ts = np.reshape(
                batch_ts, [batch_ts.shape[0], batch_ts.shape[1]])
            ...
            if(compute_optimizer):
                ...
                loss, loss_tot = criterion(outputs, batch_ys, loss1_weight=loss1_weight,
                                           loss2_weight=loss2_weight, loss3_weight=loss3_weight, loss4_weight=loss4_weight)
                print("Loss: ", loss)
    ...
    if(compute_val_loss or compute_val_pred):
        ...
        for i in range(number_batches_val):
            ...
            batch_xs, batch_ys, batch_ts = data_val_batches[
                i], labels_val_batches[i], elapsed_val_batches[i]
            ...
            if(compute_val_loss):
                ...
                total_loss, loss_tot = criterion(outputs, batch_ys, loss1_weight=loss1_weight,
                                                 loss2_weight=loss2_weight, loss3_weight=loss3_weight, loss4_weight=loss4_weight)
