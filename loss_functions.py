...
def loss_survival(outputs, batch_ys, loss1_weight=10, loss2_weight=1,
    ...
    diff_y = batch_ys - \
        torch.cat((torch.zeros(
            batch_ys.shape[0], 1, output_size).double(), batch_ys[:, :-1, :]), dim=1)
    relapse, mask_max_indices = torch.max(diff_y, dim=1)

    if(relapse.sum() > 0):
        loss2 = relapse*(1.-torch.gather(outputs,
                                         index=mask_max_indices.unsqueeze(dim=1), dim=1).squeeze(dim=1))
    ...
    if(relapse.sum() > 0):
        ...
        loss3 = relapse * \
            (torch.gather(outputs, index=indexes2.unsqueeze(dim=1), dim=1).squeeze(dim=1))
    ...
    if (len(relapse)-relapse.sum()) > 0:
        ...
        loss4 = loss4_weight * \
            torch.square(loss4).sum()/(relapse.numel()-relapse.sum())
